...
int main(int argc, char **argv) {
  ...
  snprintf(glb.socket_path_status, 32, "%s", "gui/status");
  snprintf(glb.socket_path_cmd, 32, "%s", "gui/cmd");
  ...
  // sockets threads
  pthread_t pthread_socket_status_id;
  pthread_return = pthread_create(&pthread_socket_status_id, NULL, &pthread_socket_status, (void *)&glb);
  if (pthread_return != 0) {
    fprintf(stderr, "ERROR; return code from pthread_create() is %d\n", pthread_return);
    exit(-1);
  }

  pthread_t pthread_socket_cmd_id;
  pthread_return = pthread_create(&pthread_socket_cmd_id, NULL, &pthread_socket_cmd, (void *)&glb);
  if (pthread_return != 0) {
    fprintf(stderr, "ERROR; return code from pthread_create() is %d\n", pthread_return);
    exit(-1);
  }

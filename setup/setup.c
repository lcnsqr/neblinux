...
void *pthread_temp_gist(void *arg){
  ...
  // Nos testes, o slope estável será < 1e-3 = 0.001 no domínio e imagem normalizadis 
  ...
  for (int i = 0; i < tail_points; i++) tail_x[i] = (float)i/(float)tail_points;
  ...
  while (fflush(gnuplot) == 0){
    ...
    fprintf(gnuplot, "%s\n", "plot \"-\" using 1:2 title \"Interna\" with lines lw 2, \"-\" using 1:3 title \"Sonda\" with lines lw 2, \"-\" using 1:4 title \"Aquecimento\" with lines lw 2");

    pthread_mutex_lock(&temps_mut);

    for (int j = 0; j < tail_points; j++){
      tail_core[j] = temps.core[(temps.i_core+(GRAPH_POINTS-tail_points)+j)%GRAPH_POINTS] / y_temp_max;
      tail_probe[j]   = temps.probe[(temps.i_probe+(GRAPH_POINTS-tail_points)+j)%GRAPH_POINTS] / y_temp_max;
      tail_heat[j]   = temps.heat[(temps.i_heat+(GRAPH_POINTS-tail_points)+j)%GRAPH_POINTS] / y_heat_max;
    }
    ...
    //fprintf(fd_coefs, "%5.6f\t%5.6f\t%5.6f\t%5.6f\n", coefs_core[0], coefs_core[1], coefs_probe[0], coefs_probe[1]);
    ...
    for (int j = 0; j < tail_points; j++){
      ...
      fprintf(gnuplot, "%.2f\t%.2f\t%.2f\t%.2f\n", tail_x[j], tail_core_line[j], tail_probe_line[j], tail_heat_line[j]);
    }
...
void *pthread_plot(void *arg){
  ...
  while (fflush(gnuplot) == 0){
    ...
    fprintf(gnuplot, "%s\n", "plot \"-\" using 1:2 title \"Interna\" with lines lw 2, \"-\" using 1:3 title \"Sonda\" with lines lw 2, \"-\" using 1:4 title \"Alvo\" with lines lw 2, \"-\" using 1:5 title \"Saída\" with lines lw 2");

    pthread_mutex_lock(&temps_mut);

    for (int j = 0; j < GRAPH_POINTS; j++){
      fprintf(gnuplot, "%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n", -(float)GRAPH_POINTS*GRAPH_STEP + (float)j*GRAPH_STEP, temps.core[(temps.i_core+j)%GRAPH_POINTS], temps.probe[(temps.i_probe+j)%GRAPH_POINTS], temps.target[(temps.i_target+j)%GRAPH_POINTS], temps.ex[(temps.i_ex+j)%GRAPH_POINTS]);
    }
...
void *pthread_rx_probe(void *arg){
  ...
  while (rx_bytes >= 0){
    ...
    if (rx_bytes != sizeof(float) ) continue;
...
int init_tty(int port_vapomatic){
  ...
  tty.c_cc[VTIME] = 30;    // Wait for up to 3s (30 deciseconds), returning as soon as any data is received.
  tty.c_cc[VMIN] = 0;
...
int main(int argc, char **argv){
  ...
  pthread_return = pthread_create(&pthread_rxtx_id, NULL, &pthread_rxtx, (void *)&port_vapomatic); 
  if ( pthread_return != 0 ) {
  ...
  pthread_return = pthread_create(&pthread_rx_probe_id, NULL, &pthread_rx_probe, (void *)&port_probe); 
  if ( pthread_return != 0 ) {
    fprintf(stderr, "ERROR; return code from pthread_create() is %d\n", pthread_return);
    exit(-1);
  }
  ...
  pthread_return = pthread_create(&pthread_plot_id, NULL, &pthread_plot, (void *)NULL); 
  if ( pthread_return != 0 ) {
    fprintf(stderr, "ERROR; return code from pthread_create() is %d\n", pthread_return);
    exit(-1);
  }
  ...
  pthread_return = pthread_create(&pthread_temp_gist_id, NULL, &pthread_temp_gist, (void *)NULL); 
  if ( pthread_return != 0 ) {
    fprintf(stderr, "ERROR; return code from pthread_create() is %d\n", pthread_return);
    exit(-1);
  }
...
int exec(char *cmdline){
  ...
  if ( ! strcmp("fan", tokens[0]) ){
    ...
    // Change state
    pthread_mutex_lock(&state_mut);
    stateOut.fan = ( ! strcmp("on", tokens[1]) || ! strcmp("1", tokens[1]) ) ? 1 : 0;
    state_change = 1;
    pthread_mutex_unlock(&state_mut);
  ...
  if ( ! strcmp("mkdir", tokens[0]) ){
    // Using 0777 as the directory permission (it will be combined with system's umask)
    mkdir(tokens[1], 0777);
    tokens_cleanup(tokens);
    return 0;
  }

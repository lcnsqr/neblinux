...
void Autostop::action() {
  ...
  // Reta recente da temperatura superior a cStop[0]
  // Reta recente da carga inferior a cStop[1]
  // now - targetLastChange > targetMinSec

  if ( session->state.autostop &&
       session->state.elapsed > minSec &&
       session->state.PID_enabled &&
       session->state.tempEx > session->state.tempTarget &&
       session->state.sStop[0] > session->state.cStop[0] &&
       session->state.sStop[1] < session->state.cStop[1] &&
